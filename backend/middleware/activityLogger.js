...
export default function activityLogger(req, res, next) {
  // Express restores req.path after a mounted router unwinds. Capture the
  // original API URL while it still identifies the route that was handled.
  ...
  res.on('finish', () => {
    try {
      ...
      const { resource, action } = describe(req.method, requestPath)
      ActivityLog.create({
        actorId: user._id,
        actorName: user.name,
